...
public partial class DonutChart : ComponentBase
{
    ...
    private string AccessibleName => string.IsNullOrEmpty(Title) ? "Donut chart" : Title!;
    private string AccessibleDescription => Description ?? BuildAccessibleDescription();

    private string BuildAccessibleDescription()
    {
        if (Data.Count == 0) return "Donut chart with no data.";
        ...
        return $"Donut chart of {Data.Count} segments totaling {total}. Largest is {top.Label} at {pct}%.";
    }
